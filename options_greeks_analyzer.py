...
C = {
    ...
    "put":        "#a855f7", "call":      "#d4a843",
    "net_pos":    "#34d399", "net_neg":   "#f87171",
    "vanna_net":  "#a855f7", "vanna_call":"#34d399",
    "vanna_put":  "#f87171", "charm_net": "#a855f7",
    "charm_call": "#34d399", "charm_put": "#f87171",
    "ctrl":       "#0a0a0a", "btn_off":   "#161616",
    "btn_on":     "#2a1f3d", "live":      "#34d399",
    "demo":       "#f97316",
}
...
def get_options_chain(token, symbol, spot):
    """
    Fetch with range=ALL but limit to nearest 45 days.
    This gives full $1 strike density without 502 timeouts.
    SPY: ~60 strikes x 8 expirations = ~480 contracts (manageable)
    vs range=ALL no date limit = 50,000+ contracts (502 timeout)
    """
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code in [502, 503, 504] and attempt < 2:
                w = (attempt + 1) * 5
                print(f"  ⚠️  {r.status_code} — retry in {w}s...")
                time.sleep(w)
                continue
            raise
    ...
    for side, exp_map in [("call", chain.get("callExpDateMap", {})),
        ...
        for exp_key, strikes in exp_map.items():
            ...
            for ks, contracts in strikes.items():
                ...
                rows.append({
                    "strike": K, "type": side, "dte": dte, "oi": oi,
...
def aggregate(df):
    cols = ["GEX_call","GEX_put","VannEX","VannEX_call","VannEX_put",
            "CharmEX","CharmEX_call","CharmEX_put","oi"]
    a = (df.groupby("strike")[cols].sum()
...
def print_summary(symbol, df, spot):
    ...
    print(f"  Net VannEX  ${tv/1e6:+.1f}M")
    print(f"  Net CharmEX {tc/1e3:+.1f}K")
    print()
...
def _fill_signed(ax, x, y, pos_color, neg_color, alpha=0.15):
    """
    Fill area between line and zero correctly.
    Positive regions → pos_color, Negative regions → neg_color.
    Uses interpolate=True so crossings are handled cleanly.
    """
...
def draw_gex(ax, agg, spot, symbol):
    ...
    # Limit to 50 strikes closest to spot — matches UnusualWhales default
    plot_agg = (agg.copy()
    ...
    ax.barh(strikes, net_v, height=bar_h * 1.0,
    ...
    if not pos.empty and not neg.empty:
        ...
        ax.text(-xmax * 0.96, flip + bar_h * 0.6,
                f"Γ flip ${flip:.0f}", color=C["subtext"], fontsize=7.5)
...
def draw_vanna(ax, agg, spot, symbol, split):
    ...
    if split:
        cv = agg["VannEX_call"].values / 1e3
        pv = agg["VannEX_put"].values  / 1e3
        ...
        nv    = agg["VannEX"].values / 1e3
        all_v = nv
        ax.plot(strikes, nv, color=C["vanna_net"], linewidth=2.2,
                label="Vanna", zorder=4)
        _fill_signed(ax, strikes, nv, C["net_pos"], C["net_neg"])
        if len(nv):
    ...
    # Always symmetric Y axis
    ymx = max(np.abs(all_v).max() * 1.25, 0.01)
    ax.set_ylim(-ymx, ymx)
    ...
    ax.set_ylabel("Vanna", color=C["subtext"], fontsize=8)
...
def draw_charm(ax, agg, spot, symbol, split):
    ...
    if split:
        cc = agg["CharmEX_call"].values / 1e6
        pc = agg["CharmEX_put"].values  / 1e6
        ...
        all_v = nc
        ax.plot(strikes, nc, color=C["charm_net"], linewidth=2.2,
                label="Charm", zorder=4)
        _fill_signed(ax, strikes, nc, C["net_pos"], C["net_neg"])
        if len(nc):
    ...
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(
        lambda x, _: f"{x:.0f}M" if abs(x) >= 1 else f"{x:.2f}M"
    ))
    ax.set_xlabel("Strike",    color=C["subtext"], fontsize=8)
    ax.set_ylabel("Charm", color=C["subtext"], fontsize=8)
...
def launch_dashboard(all_data):
    ...
    tk.Label(ctrl, text="● LIVE" if live else "● DEMO",
    ...
    def render(*_):
        ...
        canvas.draw()
        fig.savefig("options_greeks_dashboard.png", dpi=dpi,
    ...
    def on_scroll(event):
        ...
        zoom[0] = max(0.4, min(3.5, zoom[0] * (0.88 if event.delta > 0 else 1.12)))
        nw = int(base_w[0] * zoom[0]); nh = int(base_h[0] * zoom[0])
    ...
    for dte in [3, 7, 14, 21, 28, 45]:
        ...
        for K in np.arange(lo, hi + 1, 1.0):
            for side, call in [("call", True), ("put", False)]:
                ...
                mult = oi * 100; sign = 1 if call else -1
                rows.append({
                    "strike": round(K,2), "type": side, "dte": dte, "oi": oi,
                    "GEX_call":     g*mult*spot  if call     else 0,
                    "GEX_put":     -g*mult*spot  if not call else 0,
                    "VannEX":       sign*va*mult*spot,
                    "VannEX_call":  va*mult*spot  if call    else 0,
                    "VannEX_put":  -va*mult*spot  if not call else 0,
                    "CharmEX":      sign*ch*mult,
                    "CharmEX_call": ch*mult       if call    else 0,
                    "CharmEX_put": -ch*mult       if not call else 0,
                })
...
def main():
    ...
    if demo:
        ...
        all_data = {}
        for sym in SYMBOLS:
            spot = DEMO_SPOTS[sym]; df = generate_demo_chain(spot, sym)
            all_data[sym] = (df, spot); print_summary(sym, df, spot)
    else:
        from auth import get_valid_access_token
        token = get_valid_access_token(); all_data = {}
        for sym in SYMBOLS:
            print(f"📡  {sym}...")
            time.sleep(5)
            try:
                qr = requests.get(f"{SCHWAB_BASE}/quotes",
                                  headers={"Authorization": f"Bearer {token}"},
                                  params={"symbols": sym}, timeout=10)
                qd = qr.json()
                try:
                    inner = qd.get(sym, list(qd.values())[0] if qd else {})
                    spot  = (inner.get("quote",{}).get("lastPrice")
                             or inner.get("lastPrice") or inner.get("mark"))
                except: spot = None
                if not spot:
                    print(f"  ⚠️  No spot: {qd}"); continue
                print(f"  ${spot:.2f}")
                chain = get_options_chain(token, sym, spot)
                df    = parse_chain(chain)
                if df.empty:
                    print(f"  ⚠️  Empty"); continue
                all_data[sym] = (df, spot); print_summary(sym, df, spot)
    ...
    print("🖥️  Launching..."); launch_dashboard(all_data)
